...
\[ScriptB]TargTarg=(PGro/(R)-1/(2-LGro)+\[Kappa]*(1+(\[CapitalThorn]\[CapitalGamma]^(-\[Rho])-1)/\[Mho])^(1/\[Rho]))^(-1);(* target wealth to income ratio for the employed consumer with stakes... This is a direct application of eq:\[ScriptB]TargEStakes *)
