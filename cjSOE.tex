...
\subsection{$\urate$ as a function of $\BTargTarg$}\label{subsec:urateFromBTargTarg}
For use below, note that we can invert \eqref{eq} to express the parameters as an implicit function of target of target wealth:
\begin{eqnarray}
  \left(\frac{1-\kapShare}{\BTargTarg}\right) & = & \left[\frac{\PGro}{\Rfree}-\frac{1}{2-\LGro}+\MPCU \left(1+\frac{\PatPGro^{-\CRRA}-1}{\urate}\right)^{1/\CRRA}\right]
  \\  \left\{ \left(\frac{1-\kapShare}{\BTargTarg}\right)-\frac{\PGro}{\Rfree}+\frac{1}{2-\LGro}\right\} & = & \left[\left(1+\frac{\PatPGro^{-\CRRA}-1}{\urate}\right)^{1/\CRRA}\right]
  \\  \left\{ \left(\frac{1-\kapShare}{\BTargTarg}\right)-\frac{\PGro}{\Rfree}+\frac{1}{2-\LGro}\right\}^{\CRRA} & = & \left(1+\frac{\PatPGro^{-\CRRA}-1}{\urate}\right)
  \\  \left\{ \left(\frac{1-\kapShare}{\BTargTarg}\right)-\frac{\PGro}{\Rfree}+\frac{1}{2-\LGro}\right\}^{\CRRA}- 1& = & \frac{\PatPGro^{-\CRRA}-1}{\urate}
  \\  \left[\left\{ \left(\frac{1-\kapShare}{\BTargTarg}\right)-\frac{\PGro}{\Rfree}+\frac{1}{2-\LGro}\right\}^{\CRRA}- 1 \right]\urate & = & \PatPGro^{-\CRRA}-1
  \\  \urate & = & \left[\left\{ \left(\frac{1-\kapShare}{\BTargTarg}\right)-\frac{\PGro}{\Rfree}+\frac{1}{2-\LGro}\right\}^{\CRRA}- 1 \right]^{-1}\left(\PatPGro^{-\CRRA}-1\right)
\end{eqnarray}
